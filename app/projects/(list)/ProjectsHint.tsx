...
const ProjectsHint: React.FC<{ config?: string }> = (props) => {
  ...
  const shouldWeShowHint =
    (config.hideProjectsHint === undefined || !config.hideProjectsHint) &&
  ...
  return (
    <div className='mb-8'>
      <Hint
        showHint={showHint}
        text='Displaying all projects, sorted by the client with the most recently created project.'
        dismissCallback={() => {
          closeHint()
        }}
        dontShowCallback={() => {
